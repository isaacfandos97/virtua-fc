...
export default function liveMatch(config) {
    ...
    const component = {
        ...
        async autoSubUserTeamBeforeSkip(minute) {
            const clearSkipping = () => { this._skippingToEnd = false; };

            // Guard: endpoint, phase, bench, sub budget.
            if (!this.skipToEndUrl) { clearSkipping(); return false; }
            if (minute >= 90) { clearSkipping(); return false; }
            if (!this.benchPlayers || this.benchPlayers.length === 0) { clearSkipping(); return false; }
            if (this.substitutionsMade.length >= this.maxSubstitutions) { clearSkipping(); return false; }
            ...
            if (usedWindowMinutes.size >= this.maxWindows) { clearSkipping(); return false; }
            ...
            // Regenerate match summary with the final scores, events, and MVP,
            // then clear the skip flag so the template reveals the summary.
            if (typeof this._generateMatchSummary === 'function') {
                this.matchSummary = this._generateMatchSummary();
            }
            if (typeof this._cacheEvents === 'function') {
                this._cacheEvents();
            }
            clearSkipping();
